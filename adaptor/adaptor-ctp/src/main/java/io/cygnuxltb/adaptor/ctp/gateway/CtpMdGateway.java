...
public class CtpMdGateway implements Closeable {
     ...
     */
    CtpMdGateway(@Nonnull String gatewayId, @Nonnull CtpConfig config, @Nonnull FtdcEventPublisher publisher) {
     ...
     */
    final int SubscribeMarketData(@Nonnull String[] instruments) {
        if (isMdLogin.get()) {
            ftdcMdApi.SubscribeMarketData(instruments, instruments.length);
    ...
    class FtdcMdCallback extends AbstractFtdcCallback {
        ...
        void onMdFrontDisconnected() {
            log.warn("{}::onMdFrontDisconnected", callbackName);
            // 行情断开处理逻辑
            isMdLogin.set(false);
        ...
        void onMdFrontConnected() {
            log.info("{}::onMdFrontConnected", callbackName);
            ...
            ftdcMdApi.ReqUserLogin(field, RequestID);
