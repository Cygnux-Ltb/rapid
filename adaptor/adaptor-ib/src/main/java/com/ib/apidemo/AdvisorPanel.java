...
public class AdvisorPanel extends NewTabPanel implements IAdvisorHandler {
    ...
    private static class GroupModel extends AbstractTableModel {
        ...
        public Object getValueAt(int rowIn, int col) {
            ...
            return switch (col) {
                case 0 -> row.name();
                case 1 -> row.defaultMethod();
                case 2 -> row.accounts().toString().substring(1, row.accounts().toString().length() - 1);
                default -> null;
            };
        }
        ...
        public void setValueAt(Object val, int rowIn, int col) {
            Group row = m_groups.get(rowIn);
            switch (col) {
                case 0 -> row.name((String) val);
                case 1 -> row.defaultMethod((Method) val);
                case 2 -> row.setAllAccounts((String) val);
                default -> {
                }
            }
        }
    ...
    private static class ProfileModel extends AbstractTableModel {
        ...
        public Object getValueAt(int rowIn, int col) {
            ...
            return switch (col) {
                case 0 -> row.name();
                case 1 -> row.type();
                case 2 -> row.allocations().toString().substring(1, row.allocations().toString().length() - 1);
                default -> null;
            };
        }
        ...
        public void setValueAt(Object val, int rowIn, int col) {
            ...
            switch (col) {
                case 0 -> row.name((String) val);
                case 1 -> row.type((Type) val);
                case 2 -> row.setAllocations((String) val);
                default -> {
                }
            }
        }
