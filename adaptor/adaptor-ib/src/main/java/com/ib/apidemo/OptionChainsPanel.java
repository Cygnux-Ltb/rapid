...
//            };
            ...
            public Object getValueAt(int rowIn, int col) {
                ...
                return switch (col) {
                    case 0 -> row.m_c.lastTradeDateOrContractMonth();
                    case 1 -> row.m_c.strike();
                    case 2 -> fmtNz(row.m_bid);
                    case 3 -> fmtNz(row.m_ask);
                    case 4 -> row.m_tickAttrib;
                    case 5 -> fmtPct(row.m_impVol);
                    case 6 -> fmtNz(row.m_delta);
                    case 7 -> fmtNz(row.m_gamma);
                    case 8 -> fmtNz(row.m_vega);
                    case 9 -> fmtNz(row.m_theta);
                    case 10 -> row.m_done ? "*" : null;
                    default -> null;
                };
            }
            ...
            private class ChainRow extends TopMktDataAdapter implements IOptHandler {
                ...
                public void tickPrice(TickType tickType, double price, TickAttrib attribs) {
                    switch (tickType) {
                        case BID, DELAYED_BID -> m_bid = price;
                        case ASK, DELAYED_ASK -> m_ask = price;
                        default -> {
                        }
                    }
                }
