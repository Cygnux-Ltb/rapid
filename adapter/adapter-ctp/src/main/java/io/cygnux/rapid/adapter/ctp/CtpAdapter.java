...
import io.cygnux.rapid.gateway.ctp.NativeLibraryLoader;
...
public class CtpAdapter extends AbstractAdapter {
     ...
     */
    private CtpAdapter(@Nonnull Builder builder,
                       @Nonnull SharedEventHandler inboundHandler,
        ...
        log.info("Adaptor==[{}], Mode==[{}], isAsync==[{}], Account -> {}",
                getAdapterId(), runningMode, builder.isAsync, account);
    }
    ...
    private void processFtdcRspEvent(FtdcRspEvent event) {
        switch (event.getType()) {
            case FRONT_DISCONNECTED -> {
                ...
                var report = createAdapterReportAndUpdate(received.Source,
                        false, received.Msg);
                log.info("Received [FrontDisconnected] convert to [AdaptorReport] -> {}", report);
                aggregateEventbus.put(report);
            }
            case RSP_USER_LOGIN -> {
        ...
        try {
            if (isMdAvailable) {
                ...
                if (instrumentCodeSet.isEmpty()) {
                    ...
                    log.warn("{} -> Input instrument code array is null or empty", adapterId);
                    return false;
                }
    ...
    protected boolean directQueryOrder(@Nonnull QueryOrder query) {
        log.info("Adaptor::directQueryOrder calling, adaptorId==[{}], QueryOrder -> {}", adapterId, query);
        if (isTraderAvailable) {
            try {
                startNewFiber("QueryOrder-Worker",
                        () -> {
                            synchronized (mutex) {
                                log.info("{} -> Ready to sent ReqQryOrder, Waiting...", adapterId);
                                Sleep.millis(QUERY_INTERVAL);
                                traderGateway.nativeReqQryOrder();
                                log.info("{} -> Has been sent ReqQryOrder", adapterId);
                            }
                        });
                return true;
            } catch (Exception e) {
    ...
    protected boolean directQueryPosition(@Nonnull QueryPosition query) {
        log.info("Adaptor::directQueryPositions calling, adaptorId==[{}], QueryPosition -> {}", adapterId, query);
        if (isTraderAvailable) {
            try {
                startNewFiber("QueryPositions-Worker",
                        () -> {
                            synchronized (mutex) {
                                log.info("{} -> Ready to sent ReqQryInvestorPosition, Waiting...", adapterId);
                                Sleep.millis(QUERY_INTERVAL);
                                traderGateway.nativeReqQryInvestorPosition(query.getExchangeCode(), query.getInstrumentCode());
                                log.info("{} -> Has been sent ReqQryInvestorPosition", adapterId);
                            }
                        });
                return true;
            } catch (Exception e) {
    ...
    protected boolean directQueryBalance(@Nonnull QueryBalance query) {
        log.info("Adaptor::directQueryBalance calling, adaptorId==[{}], QueryBalance -> {}", adapterId, query);
        if (isTraderAvailable) {
            try {
                startNewFiber("QueryBalance-Worker",
                        () -> {
                            synchronized (mutex) {
                                log.info("{} -> Ready to sent ReqQryTradingAccount, Waiting...", adapterId);
                                Sleep.millis(QUERY_INTERVAL);
                                traderGateway.nativeReqQryTradingAccount();
                                log.info("{} -> Has been sent ReqQryTradingAccount", adapterId);
                            }
                        });
                return true;
            } catch (Exception e) {
    ...
    public static class Builder {
        ...
        private AdapterRunningMode runningMode = AdapterRunningMode.FULL;
        ...
        public Builder setRunningMode(AdapterRunningMode runningMode) {
        ...
        public CtpAdapter build(SharedEventHandler inboundHandler) {
            return build(inboundHandler);
        }

        public CtpAdapter build(SharedEventHandler inboundHandler, FtdcRspHandler extFtdcRspHandler) {
            return new CtpAdapter(this, inboundHandler, extFtdcRspHandler);
        }
