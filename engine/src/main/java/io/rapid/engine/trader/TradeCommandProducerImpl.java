...
public class TradeCommandProducerImpl implements TradeCommandProducer {
    ...
    public TradeCommand toTradeCommand(int targetQty, Position position) {
        ...
        if (netQty == 0) {
            /// 净持仓为0, 目标数量大于0, 开多单
            if (targetQty > 0)
                return new TradeCommand(TrdAction.OPEN, targetQty, TrdAction.INVALID, 0);
            /// 净持仓为0, 目标数量小于0, 开空单
            else
                return new TradeCommand(TrdAction.INVALID, 0, TrdAction.OPEN, abs(targetQty));
        }
        /// 净持仓大于0, 当前为多头持仓
        else if (netQty > 0) {
            /// 当前为多头持仓, 目标为继续开多单
            ...
            /// 当前为多头持仓, 目标为反向开空单
                ...
                if (netQty >= absTargetQty)
                    return new TradeCommand(TrdAction.CLOSE, absTargetQty, TrdAction.INVALID, 0);
                /// 净持仓小于目标数量, 净持仓全部平多, 差值设置为开空
                else
                    return new TradeCommand(TrdAction.CLOSE, netQty, TrdAction.OPEN, absTargetQty - netQty);
            }
        }
        /// 净持仓小于0, 当前为空头持仓
        else {
            ...
            /// 当前为空头持仓, 目标为反向开多单
                ...
                if (absNetQty >= targetQty)
                    return new TradeCommand(TrdAction.INVALID, 0, TrdAction.CLOSE, abs(targetQty));
                /// 净持仓小于目标数量, 净持仓全部平空, 差值设置为开多
                else
                    return new TradeCommand(TrdAction.OPEN, targetQty - absNetQty, TrdAction.CLOSE, absNetQty);
